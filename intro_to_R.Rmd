...
subtitle:   "Université Côte d'Azur - M2"
...
- 08th of October 10-12

- 15th of October 10-12

- 22nd of October 10-12

- 05th of November 10-12 (probably need to move this one)

- 12th of November 10-12

- 19th of November 10-12
...
- slides <https://github.com/mattiaguerini/intro_to_R>
...
We want to build a function called `prices_info` that perform the following operations for a series of asset prices:

* create a vector called `average` by computing the mean for each series

* create a vector called `volatility` by computing the standard deviation for each series

* create a vector called `mu_large` which is `TRUE` if $\mu >= 0$ and `FALSE` $\mu < 0$

* create a vector called `sd_large` which is `TRUE` if $\sigma >= 1$ and `FALSE` $\sigma < 1$ below 0

* create a vector called `both_large` which is `TRUE` only if $\mu >= 0$ and $\sigma >= 1$

* return these five vectors in a list

The asset prices are all random normals numbers and can be generated as follows:
```{r exercise_control_flows, include=T, echo=T}
set.seed(0)
asset_prices <- matrix(data = rnorm(1000), nrow = 10, byrow = T)
```
\normalsize
...
## An empty canvas
\scriptsize
```{r midwest_scatter0, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
library(tidyverse)
...
## Decide the type of graph (scatter plot)
\scriptsize
```{r midwest_scatter1, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
  geom_point()
```
\normalsize
...
\scriptsize
```{r midwest_scatter2, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
  geom_point() + 
...
\scriptsize
```{r midwest_scatter3, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
  geom_point() + 
  geom_smooth(method = "lm") +
...
\scriptsize
```{r midwest_scatter4, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
  geom_point() + 
  geom_smooth(method = "lm") +
  coord_cartesian(xlim=c(0,0.1), ylim=c(0, 1000000)) + 
...
## Change colour/size 
\scriptsize
```{r midwest_scatter5, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
...
## Change colour/size to reflect specific categories
\scriptsize
```{r midwest_scatter6, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
...
## Change colour/size to reflect specific categories (plus `color_brewer`)
\scriptsize
```{r midwest_scatter7, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
...
## Change theme
\scriptsize
```{r midwest_scatter8, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
library(RColorBrewer)
...
## Grouping and grid
\scriptsize
```{r midwest_scatter9, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest, aes(x = area, y = poptotal)) +
  geom_point(aes(col = state)) + 
  geom_smooth(method = "lm") +
...
## Prepare dataframe for a column plot
\scriptsize
```{r midwest_bar0, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
...
## Plot the graph
\scriptsize
```{r midwest_bar1, include=T, echo=T, warning=F, message=F, out.width = "60%", fig.align="center"}
ggplot(data = midwest_state, aes(x = state, y = percasian, 
                                 col = state, fill = state)) +
  geom_col(alpha = 0.2) + 
  geom_point(size = 5) + 
  scale_colour_brewer(palette = "Set1") + 
